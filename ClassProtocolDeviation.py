...
class ProtocolDeviation(Protocol504, Participiant504):
    add = 0
    update = 0
    ...
    def get_refusalFact(self):
        d = UtilsFunctions.get_el(self.protocol, 'protocolInfo', 'refusalFact', 'foundation',
                                  'name') or UtilsFunctions.get_el(
                self.protocol, 'protocolInfo', 'protocolLot', 'refusalFact', 'foundation', 'name')
        m = UtilsFunctions.get_el(self.protocol, 'protocolInfo', 'refusalFact', 'foundation',
                                  'code') or UtilsFunctions.get_el(
                self.protocol, 'protocolInfo', 'protocolLot', 'refusalFact', 'foundation', 'code')
        t = UtilsFunctions.get_el(self.protocol, 'protocolInfo', 'refusalFact', 'explanation') or UtilsFunctions.get_el(
                self.protocol, 'protocolInfo', 'protocolLot', 'refusalFact', 'explanation')
        return f"{d} | {m} | {t}".strip()

    def get_print_form_ext(self):
        d = UtilsFunctions.get_el(self.protocol, 'attachmentsInfo', 'attachmentInfo', 'url')
        if d:
            return d
...
def parserProtocolDeviation(doc, path_xml, filexml, reg, type_f):
    p = ProtocolDeviation(doc, filexml)
    ...
    ref_fact = p.get_refusalFact()
    cur.execute(
            f"""INSERT INTO {PREFIX}auction_end_protocol SET id_protocol = %s, protocol_date =  %s, purchase_number = %s, 
                            url = %s, print_form = %s, xml = %s, type_protocol = %s, cancel = %s, lot_number = %s,  abandoned_reason_name = %s, dop_info = %s, refusal_fact = %s""",
            (id_protocol, protocol_date, purchase_number, url, print_form, xml, type_f, cancel_status, lot_number,
             abandoned_reason_name, dop_info, ref_fact))
    id_p = cur.lastrowid
    if not id_p:
        logging_parser('Не получили id', xml)
    if updated:
        ProtocolDeviation.update += 1
    else:
        ProtocolDeviation.add += 1
