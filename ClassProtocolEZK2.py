...
class ProtocolEZK2(Protocol504, Participiant504):
    ...
    def test_abandoned_reason_name(self):
        d = UtilsFunctions.get_el(self.protocol, 'protocolInfo', 'abandonedReason')
        if type(d) == list:
            UtilsFunctions.logging_parser('множество abandoned_reason', self.xml)

    def get_lot_number(self):
        d = UtilsFunctions.get_el(self.protocol, 'protocolInfo', 'lotNumber')
        if not d:
            d = 1
        return d

    def get_result_zk(self, application):
        d = UtilsFunctions.get_el(application, 'admittedInfo', 'appAdmittedInfo', 'resultType')
        return d
