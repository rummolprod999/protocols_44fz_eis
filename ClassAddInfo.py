...
class ProtocolAddInfo(Protocol, Participiant):
    add = 0
    update = 0
...
def parserAddInfo(doc, path_xml, filexml, reg, type_f):
    p = ProtocolAddInfo(doc, filexml)
    ...
    if updated:
        ProtocolAddInfo.update += 1
    else:
        ProtocolAddInfo.add += 1
    cur.close()
    con.close()
