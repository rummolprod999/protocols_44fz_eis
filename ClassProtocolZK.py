...
class ProtocolZK(Protocol, Participiant):
    ...
    def test_abandoned_reason_name(self):
        d = UtilsFunctions.get_el(self.protocol, 'protocolLot', 'abandonedReason')
        if type(d) == list:
            UtilsFunctions.logging_parser('множество abandoned_reason', self.xml)

    def get_lot_number(self):
        d = UtilsFunctions.get_el(self.protocol, 'protocolLot', 'lotNumber')
        if not d:
            d = 1
        return d

    def get_result_zk(self, application):
        d = UtilsFunctions.get_el(application, 'admittedInfo', 'resultType')
        return d

    def get_price(self, application):
        d = UtilsFunctions.get_el(application, 'price')
        return d
